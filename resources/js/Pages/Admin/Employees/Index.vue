<template>
    <AdminLayout>
        ...
        <form class="mb-4 grid grid-cols-1 gap-3 rounded-lg bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-6" @submit.prevent="applyFilters">
            ...
            <select v-model="filters.position_id" class="rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                <option value="">Semua Jabatan</option>
                <option v-for="position in positions" :key="position.id" :value="position.id">{{ position.name }}</option>
            </select>
            <select v-model="filters.employment_status_id" class="rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
        ...
        <div v-if="importModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="importModal = false">
            <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
                ...
                <p class="mb-4 text-sm text-gray-500">
                    Unduh <a :href="route('admin.employees.export', { template: 1 })" class="text-emerald-700 hover:underline">template impor</a>,
                ...
                <form @submit.prevent="submitImport" class="space-y-4">
                    <input type="file" accept=".xlsx,.xls,.csv" @change="(e) => { importForm.file = e.target.files[0]; }"
                           class="block w-full text-sm text-gray-600 file:mr-3 file:rounded-md file:border-0 file:bg-emerald-50 file:px-3 file:py-2 file:text-sm file:font-medium file:text-emerald-700 hover:file:bg-emerald-100">
                    <p v-if="importForm.errors.file" class="mt-1 text-xs text-red-600">{{ importForm.errors.file }}</p>
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="importModal = false" class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Batal</button>
                        <button type="submit" :disabled="importForm.processing" class="rounded-md bg-emerald-700 px-4 py-2 text-sm text-white hover:bg-emerald-800 disabled:opacity-50">Pratinjau</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    <tr v-for="employee in employees.data" :key="employee.id">
                        ...
                        <td class="px-4 py-3 text-right">
                            <Link :href="route('admin.employees.show', employee.id)" class="text-emerald-700 hover:underline">Buka</Link>
                        </td>
                    </tr>
